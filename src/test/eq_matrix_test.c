...
START_TEST(eq_matrix_norm_true) {
...
START_TEST(eq_matrix_norm_false) {
  matrix_t m = {0};
  const int rows = rand() % 100 + 1;
  const int cols = rand() % 100 + 1;
  s21_create_matrix(rows, cols, &m);
...
START_TEST(eq_matrix_uninitialized) {
  matrix_t A = {0};
  matrix_t B = {0};
...
Suite *suite_eq_matrix(void) {
  ...
  tcase_add_loop_test(tc, eq_matrix_norm_false, 0, 100);
  tcase_add_loop_test(tc, eq_matrix_norm_true, 0, 100);

  tcase_add_test(tc, eq_matrix_uninitialized);
  tcase_add_test(tc, zero_matrix);
  tcase_add_test(tc, double_matrix_1);
  tcase_add_test(tc, double_matrix_2);
  tcase_add_test(tc, double_matrix_3);
  tcase_add_test(tc, double_matrix_4);
  tcase_add_test(tc, precision_test);
